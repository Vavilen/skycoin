...
func LoadJSON(filename string, thing interface{}) error {
	...
	defer func() {
		errInner := file.Close()
		if errInner != nil {
			logger.Warning("Closing file %s in LoadJSON failed: %v", filename, errInner)
		}
	}()
...
func SaveJSONSafe(filename string, thing interface{}, mode os.FileMode) error {
	...
	if err != nil {
		errInner := os.Remove(filename)
		if errInner != nil {
			logger.Warning("Removing file %s failed: %v", filename, errInner)
		}
	}
	return err
}
