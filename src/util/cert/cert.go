...
func GenerateCert(certFile, keyFile, host, organization string, rsaBits int,
	...
	defer func() {
		var errDefer error
		if errDefer = certOut.Close(); errDefer != nil {
			logger.Warning("Closing certFile %s in GenerateCert failed: %v", certFile, errDefer)
		}
	}()
	err = pem.Encode(certOut, &pem.Block{Type: "CERTIFICATE", Bytes: derBytes})
	if err != nil {
		return fmt.Errorf("Failed to pem.Encode for certFile: %s :%v", certFile, err)
	}
	...
	defer func() {
		var errDefer error
		if errDefer = keyOut.Close(); errDefer != nil {
			logger.Warning("Closing generated file %s in GenerateCert failed: %v", keyFile, errDefer)
		}
	}()
	if err = pem.Encode(keyOut, &pem.Block{Type: "RSA PRIVATE KEY",
